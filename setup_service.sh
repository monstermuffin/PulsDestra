...
if [ ! -f "$SCRIPT_DIR/config.yaml" ]; then
    echo "❌ config.yaml not found. Please create it from config_example.yaml first."
    exit 1
fi
...
echo ""
echo "⚠️  IMPORTANT: You need to log out and back in (or reboot) for group changes to take effect!"
